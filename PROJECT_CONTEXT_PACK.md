...
- [ ] Required next packet: add an accessible second public discovery lane that preserves the directory-to-website contact policy; the 2026-03-17 AIHA lane-yield pass measured `usable_only=0`, `packet_eligible=0`, and `selected=0` for site-contact-only firms across `TX,CA,FL`, and only `selected_row_count=1` from `candidate_count_before_filters=21` against `raw_target=30` / `gap_total=107`, so AIHA is valid but not sufficient. `BLUEBOOK` stays nondefault until listing access is operationally approved.
...
- 2026-03-17: Instrumented AIHA lane-yield tracking from generator snapshot through AI-assist packet selection so replays can measure site-contact-only usable, generator-accepted, review-eligible, packet-eligible, and selected counts by state and run. Evidence: `outreach/run_prospect_generation.py`, `tools/dump_prospect_ai_assist_review.py`, `tests_outreach_generation.py`, `tests_tools_prospect_ai_assist.py`.
