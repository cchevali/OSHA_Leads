...
class TestProspectGeneration(unittest.TestCase):
    ...
    def test_autogrow_enabled_backlog_targeted_and_deterministic_slice(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            called_states = [str(call.kwargs.get("state") or "") for call in mocked_fetch.call_args_list]
            self.assertEqual(called_states, ["TX", "CA", "FL"])
            for call in mocked_fetch.call_args_list:
                self.assertEqual(call.kwargs["max_pages"], 6)
                self.assertEqual(call.kwargs["sleep_ms"], 800)
            ...
            self.assertIn("GENERATOR_AUTOGROW_TOTAL_STATES=3", out)
            self.assertIn("GENERATOR_AUTOGROW_TOTAL_ACCEPTED=1", out)
            self.assertIn("GENERATOR_AUTOGROW_STATE=TX backlog_current=0 new_needed=60 aiha_candidate=2 aiha_accepted=0", out)
            self.assertIn("GENERATOR_AUTOGROW_STATE=CA backlog_current=0 new_needed=60 aiha_candidate=2 aiha_accepted=0", out)
            self.assertIn("GENERATOR_AUTOGROW_STATE=FL backlog_current=59 new_needed=1 aiha_candidate=2 aiha_accepted=1", out)
    ...
    def test_safety_net_forces_autogrow_for_depleted_state_when_disabled(self):
        from outreach import crm_store
        from outreach import run_prospect_generation as generator
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            try:
                ...
                conn.execute(
                    ...
                    ("tx_contacted", now, "2026-02-18_TX"),
                )
                conn.execute(
                    """
                    INSERT INTO prospects(
            ...
            mocked_fetch_result = {
                "rows": [
                    {
                        "email": "safetytx@example.com",
                        "state": "TX",
                        "firm": "Safety TX",
                        "title": "EHS Consultant",
                        "city": "Houston",
                        "source": "aiha_consultants_listing:62-63",
                    }
                ],
                "cache_used": False,
                "cache_age_days": 0,
            ...
            with mock.patch.dict(os.environ, env, clear=False):
                with mock.patch(
                    ...
                    with redirect_stdout(buf):
                        rc = generator.main(["--dry-run", "--for-date", "2026-02-19"])

            self.assertEqual(rc, 0)
            mocked_fetch.assert_called_once()
            self.assertEqual(str(mocked_fetch.call_args.kwargs.get("state") or ""), "TX")

            out = buf.getvalue()
            self.assertIn("WARN_AUTOGROW_SAFETY_NET_FORCED states=TX sources=AIHA", out)
            self.assertIn("GENERATOR_AUTOGROW_ENABLED=1", out)
            self.assertIn("GENERATOR_AUTOGROW_SAFETY_NET_FORCED=1", out)
            self.assertIn("GENERATOR_AUTOGROW_SAFETY_NET_STATES=TX", out)
            self.assertIn("GENERATOR_AUTOGROW_TOTAL_ACCEPTED=1", out)
            self.assertIn("GENERATOR_AUTOGROW_STATE=TX backlog_current=0 new_needed=60 aiha_candidate=1 aiha_accepted=1", out)
            self.assertIn("GENERATOR_AUTOGROW_STATE=CA backlog_current=1 new_needed=59 aiha_candidate=0 aiha_accepted=0", out)

    def test_safety_net_can_be_disabled_explicitly(self):
        from outreach import crm_store
        from outreach import run_prospect_generation as generator
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            out = buf.getvalue()
            self.assertNotIn("WARN_AUTOGROW_SAFETY_NET_FORCED", out)
            self.assertIn("GENERATOR_AUTOGROW_ENABLED=0", out)
            self.assertIn("GENERATOR_AUTOGROW_SAFETY_NET_FORCED=0", out)
            self.assertIn("GENERATOR_AUTOGROW_SAFETY_NET_STATES=none", out)
            self.assertIn("GENERATOR_AUTOGROW_STATE=TX backlog_current=0 new_needed=60 aiha_candidate=0 aiha_accepted=0", out)

    def test_seed_feed_filters_to_configured_states_only(self):
        with tempfile.TemporaryDirectory() as d:
            data_dir = Path(d) / "data"
            ...
            p = self._run([], {"DATA_DIR": str(data_dir), "OUTREACH_STATES": "TX"})
            self.assertEqual(p.returncode, 0, msg=p.stderr + "\n" + p.stdout)
            ...
            with open(out_path, "r", newline="", encoding="utf-8") as f:
                for row in csv.DictReader(f):
                    states_seen.add((row.get("state") or "").strip().upper())
            self.assertTrue(states_seen, msg="expected at least one generated row")
            self.assertEqual(states_seen, {"TX"})

    def test_new_state_can_autogrow_without_crash(self):
        from outreach import run_prospect_generation as generator
        ...
        with tempfile.TemporaryDirectory() as d:
                ...
                "rows": [
                    {
                        ...
                        "title": "EHS Consultant",
                        "city": "New York",
                        "source": "aiha_consultants_listing:40-41",
                    }
                ],
                "cache_used": False,
                "cache_age_days": 0,
                "cache_path": data_dir / "prospect_generation" / "cache" / "aiha" / "state_NY.json",
                "pages_fetched": 1,
                "parse_mode": "TEXT_CONTAINER",
                "diagnostics_path": None,
            }
            ...
            self.assertEqual(rc, 0)
            mocked_fetch.assert_called_once()
            self.assertEqual(str(mocked_fetch.call_args.kwargs.get("state") or ""), "NY")
            out = buf.getvalue()
            self.assertIn("GENERATOR_AUTOGROW_STATE=NY backlog_current=0 new_needed=60 aiha_candidate=1 aiha_accepted=1", out)
