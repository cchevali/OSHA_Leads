...
class TestProspectGeneration(unittest.TestCase):
    ...
    def test_print_config_includes_crawl4ai_and_new_source_availability(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with mock.patch.dict(os.environ, env, clear=False):
                with mock.patch("outreach.run_prospect_generation.scraper_engine.probe_crawl4ai_runtime", return_value={"crawl4ai_installed": False, "playwright_browsers_installed": False, "error_reason": "missing"}):
                    with mock.patch(
                        ...
                        side_effect=[
                            {"source": "BCSP", "available": False, "reason": "crawl4ai_not_installed"},
            ...
            self.assertIn("BCSP_available=NO reason=crawl4ai_not_installed", out)
    ...
    def test_crawl4ai_missing_warns_and_state_lic_still_runs(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            bcsp_fail = {
                ...
                "error": "bcsp_unavailable err=crawl4ai_not_installed",
            }
            state_lic_ok = {
                "rows": [
            ...
            self.assertEqual(rc, 0)
            out = buf.getvalue()
            self.assertIn("WARN_CRAWL4AI_NOT_INSTALLED source=bcsp state=TX", out)
    ...
    def test_generator_doctor_aggregate_warning_level(self):
        ...
        with mock.patch.dict(os.environ, env, clear=False):
            with mock.patch("outreach.run_prospect_generation.scraper_engine.probe_crawl4ai_runtime", return_value={"crawl4ai_installed": False, "playwright_browsers_installed": False, "error_reason": "missing"}):
                with mock.patch("outreach.run_prospect_generation.scraper_engine.probe_source_availability", side_effect=[{"available": False, "reason": "crawl4ai_not_installed"}, {"available": True, "reason": "http_api"}]):
