...
class TestScheduleCheck(unittest.TestCase):
    ...
    def test_schedule_ok_from_xml_interactive_background_mode(self):
        xml_text = (FIXTURES_DIR / "task_action_ok.xml").read_text(encoding="utf-8")
        actual = extract_exec_action(xml_text)
        expected = build_task_action(r"C:\dev\OSHA_Leads\run_wally_trial_daily.bat")
        ...
        buf = io.StringIO()
        with redirect_stdout(buf):
            verify_schedule_action_from_actual(expected, actual, "Password", "Interactive/Background")

        self.assertEqual(buf.getvalue().strip(), f"SCHEDULE_OK /TR={expected} /LOGON_MODE=Interactive/Background")
