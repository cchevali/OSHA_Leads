...
- [ ] Provision Gmail OAuth client JSON and bootstrap token for inbound triage on the canonical PC: create `secrets/gmail_credentials.json`, install Gmail deps if needed, run `py -3 inbound_inbox_triage.py --dry-run --since-hours 1`, then run a single `py -3 inbound_inbox_triage.py --run-once` to create `secrets/gmail_token.json`. The Ops Console Inbox / Requests screen now shows the exact status, paths, and next command.
