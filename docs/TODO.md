...
- [ ] If you intentionally choose the Gmail inbound backend instead of the Zoho/IMAP path, provision Gmail OAuth client JSON and bootstrap token on the canonical PC: create `secrets/gmail_credentials.json`, install Gmail deps if needed, run `py -3 inbound_inbox_triage.py --dry-run --since-hours 1`, then run a single `py -3 inbound_inbox_triage.py --run-once` to create `secrets/gmail_token.json`. For the normal Zoho setup, use `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\set_outreach_env.ps1 -InboundBackend imap -SyncInboundImapFromBounce` instead; the Ops Console Inbox / Requests screen now shows the exact status, paths, and next command.
...
- 2026-03-24: Runtime tick now owns console freshness support via weekday `ops_snapshot_daily` (`09:30`) and `outreach_cleanup_daily` (`09:45`) jobs, and the Ops Console Inbox / Requests screen now surfaces backend-aware Gmail/IMAP setup status with exact local paths and bootstrap commands instead of a binary Gmail check. Evidence: `outreach/run_runtime_tick.py`, `ops_console/app.py`, `tests_run_runtime_tick.py`, `tests_ops_console.py`, `docs/RUNBOOK.md`, `docs/ARCHITECTURE.md`.
- 2026-03-24: Added an operator-safe inbound IMAP config path so Zoho mailbox settings already saved under `BOUNCE_IMAP_*` can be copied into `INBOUND_BACKEND=imap` plus `IMAP_*` through `scripts\set_outreach_env.ps1`, and updated the console/scheduled wrapper/docs to treat Gmail as optional instead of required for Zoho setups. Evidence: `scripts/set_outreach_env.ps1`, `ops_console/app.py`, `inbound_inbox_triage.py`, `scripts/scheduled/run_inbound_triage.ps1`, `outreach/run_runtime_tick.py`, `tests_ops_console.py`, `tests_inbound_imap.py`, `tests_run_inbound_triage_wrapper.py`, `docs/RUNBOOK.md`, `docs/ARCHITECTURE.md`.
