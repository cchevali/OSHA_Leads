...
- 2026-03-10: Cut over morning scheduling so the self-hosted `runtime-tick-selfhosted.yml` path is the canonical live executor, while Task Scheduler morning wrappers install as recovery-only/disabled and status emits runner/Python/topology health tokens. Evidence: `scripts/install_scheduled_tasks.ps1`, `outreach/run_runtime_tick.py`, `tests_install_scheduled_tasks.py`, `tests_run_runtime_tick.py`.
