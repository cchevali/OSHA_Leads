...
1. Daily prospect replenishment: `run_prospect_replenish_daily.py` runs deterministic pipeline stages in order:
   ...
   - Wrapper default env posture is `PROSPECT_AUTOGROW_ENABLED=1`, `PROSPECT_AUTOGROW_SOURCES=AIHA,BLUEBOOK`, `PROSPECT_AUTOGROW_SAFETY_NET_ENABLED=1`, `PROSPECT_AI_ASSIST_REVIEW_ENABLED=1`, and `PROSPECT_AI_ASSIST_REVIEW_RAW_TARGET=30` when these keys are unset.
   ...
   - Canonical primary consultant discovery sources (`AIHA`, `BLUEBOOK`) use a directory-to-website public-contact path: keep a valid non-free source email when present, otherwise crawl the source-provided website (`/`, `/contact`, `/contact-us`, `/about`, `/about-us`, `/team`, `/our-team`) for a public business email, otherwise leave email blank.
   - Guessed domains, guessed emails, and Hunter/provider lookups remain noncanonical/manual or secondary-lane tooling; they are not part of the canonical `AIHA,BLUEBOOK` default lane.
