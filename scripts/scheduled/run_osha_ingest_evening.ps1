...
$summaryResult = Write-RuntimeRunSummary `
  ...
  -Intent 'write' `
  -DryRun:$false `
  -ExitCode $(if ($ingestExitCode -ne 0 -or $signalsDumpExitCode -ne 0) { 1 } else { 0 }) `
  ...
  -ExtraArtifactPaths @(@($signalsDumpOutputPath) | Where-Object { ([string]$_).Trim() }) `
  -EmitLine ${function:Write-TaskLine}
# RUN_SUMMARY_JSON_PATH= / RUN_SUMMARY_TEXT_PATH= emitted above via Write-RuntimeRunSummary.

if ($ingestExitCode -ne 0 -or $signalsDumpExitCode -ne 0) {
  exit 1
}
exit 0
