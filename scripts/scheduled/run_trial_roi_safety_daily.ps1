...
$trialSubscriberKey = "roi_safety_trial"
$trialExitCode = 1
$preflight = $null
...
$runtimeTickState = Test-RuntimeTickDailySlotAlreadyCompleted `
  -RepoRoot $repoRoot `
  -JobName 'trial_roi_safety_daily' `
  -NowLocal $startLocal `
  -EmitLine ${function:Add-BootstrapLine}
...
$taskLogPath = New-RuntimeTaskLogPath -TaskLogRoot $taskLogDir -WrapperName 'OSHA_Trial_ROI_Safety_Daily' -RunId $runId
...
$summaryResult = Write-RuntimeRunSummary `
  -RepoRoot $repoRoot `
  -WrapperName 'OSHA_Trial_ROI_Safety_Daily' `
