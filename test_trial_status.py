...
class TestTrialStatus(unittest.TestCase):
        ...
        *,
        ...
        with crm_light.open_conn(db_path) as conn:
            ...
            crm_light.upsert_subscriber(
                ...
                email=email or f"{subscriber_key}@example.com",
    ...
    def test_live_concurrent_lock_skips_before_deliver(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp)
            ...
            try:
                ...
                locked_handle = runtime_guard.RuntimeLockHandle(
                    acquired=False,
                    path=r"C:\locks\trial_daily_facs_trial_2026-02-20.json",
                    name="trial_daily_facs_trial_2026-02-20",
                    token="",
                    metadata={
                        "pid": 9876,
                        "hostname": "scheduler-host",
                        "acquired_at_utc": "2026-02-20T13:00:00+00:00",
                    },
                    reason="locked",
                    stale_reclaimed=False,
                )
                orig_lock = run_trial_daily.acquire_runtime_lock
                orig_deliver = run_trial_daily._run_deliver_daily
                orig_split = run_trial_daily._detect_split_ledger_conflict
                try:
                    run_trial_daily.acquire_runtime_lock = lambda *_a, **_k: locked_handle  # type: ignore[assignment]
                    run_trial_daily._run_deliver_daily = lambda *_a, **_k: (_ for _ in ()).throw(AssertionError("deliver should not run when locked"))  # type: ignore[assignment]
                    run_trial_daily._detect_split_ledger_conflict = lambda *_a, **_k: {"conflict": False}  # type: ignore[assignment]
                    out = io.StringIO()
                    with contextlib.redirect_stdout(out):
                        code = run_trial_daily.run_trial_daily(
                            subscriber_key="facs_trial",
                    ...
                    run_trial_daily.acquire_runtime_lock = orig_lock  # type: ignore[assignment]
                    run_trial_daily._run_deliver_daily = orig_deliver  # type: ignore[assignment]
                    run_trial_daily._detect_split_ledger_conflict = orig_split  # type: ignore[assignment]
                self.assertEqual(code, 0)
                self.assertIn("TRIAL_SKIP_CONCURRENT_RUN=1 subscriber_key=facs_trial", out.getvalue())
    ...
    def test_conversion_draft_template_plain_text_and_required_content(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIn(
                    "You've been receiving the weekday OSHA activity digest for Texas Triangle over the past couple of weeks. Wanted to check in before the trial ends.",
                    text,
                )
                self.assertIn(
                    '1. Reply "go" if you\'d like me to confirm coverage first. To activate immediately, use the secure checkout link below.',
                    text,
                )
                self.assertIn("2. Or activate here:", text)
                self.assertIn("Activate secure checkout:", text)
                self.assertIn(
                    "If you'd like me to double-check coverage first, reply with the metros you care about and I'll confirm them before you activate.",
                    text,
                )
                ...
                self.assertNotIn('P.S. "0 new"', text)
    ...
    def test_conversion_draft_strips_internal_segment_codes_from_subject_and_body(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIn(
                    "You've been receiving the weekday OSHA activity digest for CA, OR, and WA over the past couple of weeks. Wanted to check in before the trial ends.",
                    text,
                )
    ...
    def test_conversion_draft_html_checkout_link_uses_clean_anchor_text(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIn(">Activate secure checkout<", html)
                self.assertEqual(html.count("https://example.com/activate"), 1)
                self.assertNotIn("Activate secure checkout: https://example.com/activate", html)
            finally:
                if old_data_dir is None:
                    os.environ.pop("DATA_DIR", None)
                else:
    ...
    def test_render_conversion_email_text_falls_back_when_display_label_missing(self) -> None:
        ...
        self.assertIn(
            "You've been receiving the weekday OSHA activity digest over the past couple of weeks. Wanted to check in before the trial ends.",
            text,
        )
        self.assertNotIn("for  over the past couple of weeks", text)

    def test_conversion_draft_plus_addressed_email_uses_neutral_greeting(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp)
            data_dir = base / "data_dir"
            old_data_dir = os.environ.get("DATA_DIR")
            ...
            try:
                ...
                self._seed_trial(
                    db,
                    subscriber_key="conversion_smoke",
                    start_date="2026-02-04",
                    sends_limit=14,
                    email="cchevali+oshasmoke@gmail.com",
                )
                code = run_trial_admin.main(["conversion-draft", "--subscriber-key", "conversion_smoke"])
                self.assertEqual(code, 0)
                artifact = data_dir / "trials" / "conversion_smoke" / "conversion_email.txt"
                text = artifact.read_text(encoding="utf-8")
                self.assertIn("\n\nHi,\n\n", text)
                self.assertNotIn("Hi Cchevali+Oshasmoke,", text)
            finally:
                if old_data_dir is None:
                    os.environ.pop("DATA_DIR", None)
                else:
