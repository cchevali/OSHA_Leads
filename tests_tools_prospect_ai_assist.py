...
class TestProspectAiAssistTools(unittest.TestCase):
    ...
    def test_dump_writes_packetized_artifacts_with_seed_candidates(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn(
                "firm,website,state,city,phone,address,seed_source,seed_source_url,source_record_id,license_number,seed_id",
                prompt_text,
            )
    ...
    def test_dump_keeps_state_lic_blank_website_rows_observable_before_caps(self):
        with tempfile.TemporaryDirectory() as d:
            tmp = Path(d)
            data_dir = tmp / "runtime"
            output_dir = tmp / "prospect_ai_assist"
            ...
            self.assertIn("AI_ASSIST_DUMP_SOURCE_STATE_LIC_SELECTED=0", text)
            self.assertIn("AI_ASSIST_DUMP_STATE_LIC_CAP_PERCENT=30", text)
            self.assertIn("AI_ASSIST_DUMP_STATE_LIC_CAP_LIMITED_COUNT=2", text)

            packet_dir = output_dir / "prospect_ai_assist_review_20260315_packets"
            review_text = (output_dir / "prospect_ai_assist_review_20260315.txt").read_text(encoding="utf-8")
            packet_status = (packet_dir / "packet_status.txt").read_text(encoding="utf-8")
            ...
            self.assertEqual(manifest["state_lic_cap_percent"], 30)
            self.assertEqual(manifest["state_lic_cap_limited_count"], 2)
            self.assertTrue((packet_dir / "seed_index.json").exists())
    ...
    def test_dump_excludes_tx_environmental_air_conditioning_rows_from_packets(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(
                manifest["state_lic_hard_negative_class_breakdown"],
                {"tx_environmental_air_conditioning": 1},
            )
            self.assertIn("NO PACKETS TODAY", packet_status)
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self._write_cache_rows(
                ...
                [
                    ...
                    {
                        "business_name": "Metro Systems Group",
                        "state": "TX",
                        "source": "STATE_LIC",
                    },
                ],
            ...
            self.assertEqual(manifest["excluded_bad_firm"], 1)
            self.assertEqual(manifest["excluded_state_mismatch"], 1)
            self.assertEqual(manifest["excluded_missing_minimum_locator"], 0)
            self.assertEqual(manifest["excluded_state_lic_blank_website_no_positive_evidence"], 1)
            self.assertEqual(manifest["excluded_state_lic_fit_mismatch"], 0)
            self.assertEqual(
                manifest["stage_counts_by_source"]["STATE_LIC"],
                {
            ...
            self.assertIn("AI_ASSIST_DUMP_SOURCE_STATE_LIC_EXCLUDED_STATE_LIC_BLANK_WEBSITE_NO_POSITIVE_EVIDENCE=1", text)

    def test_dump_drops_state_lic_cap_to_twenty_percent_when_accept_rate_is_low(self):
        with tempfile.TemporaryDirectory() as d:
            tmp = Path(d)
            data_dir = tmp / "runtime"
            output_dir = tmp / "prospect_ai_assist"
            ...
            try:
                ...
                for idx in range(10):
                    conn.execute(
                        ...
                        ) VALUES (?, ?, 'TX', ?, ?, '', '', '', '', ?, '[]', 0, '', 'pending_verification', '', '', ?, ?, '', 'STATE_LIC', 'STATE_LIC', '', ?, ?, 'electrical contractor', '', '[\"safety\"]', '[]', '')
                        """,
                        (
                            "2026-03-14_AIASSIST",
                            f"candidate_{idx}",
                            "accept" if idx == 0 else "reject",
                            f"Feedback Firm {idx}",
                            f"feedback{idx}@example.com",
                            now_iso,
                            now_iso,
                            f"tdlr:FB-{idx}",
                            f"FB-{idx}",
                ...
                [
                    {
                        ...
                        "state": "TX",
                        "source": "aiha_consultants_listing:41",
                    },
                    {
                        "firm": "Bravo Safety",
                        "website": "https://bravo.example.com",
                        "state": "TX",
                        "source": "aiha_consultants_listing:42",
                    },
                    {
                        "firm": "Charlie Safety",
                        "website": "https://charlie.example.com",
                        "state": "TX",
                        "source": "aiha_consultants_listing:43",
                    },
                    {
                        "firm": "Delta Safety",
                        "website": "https://delta.example.com",
                        "state": "TX",
                        "source": "aiha_consultants_listing:44",
                    },
                    {
                        "firm": "Echo Safety",
                        "website": "https://echo.example.com",
                        "state": "TX",
                        "source": "aiha_consultants_listing:45",
                    },
                ],
            )
            self._write_cache_rows(
                ...
                [
                    {
                        "business_name": "Foxtrot Safety Consulting",
                        "state": "TX",
                        "business_city_state_zip": "Austin TX 78701",
                        "license_number": "SC-401",
                        "source_detail": "tdlr:SC-401",
                        "source": "STATE_LIC",
                    },
                    {
                        "business_name": "Golf Safety Compliance",
                        "state": "TX",
                        "business_city_state_zip": "Dallas TX 75001",
                        "license_number": "SC-402",
                        "source_detail": "tdlr:SC-402",
                        "source": "STATE_LIC",
                    },
                ],
            )
            ...
            text = out.getvalue()
            manifest = json.loads(
                (output_dir / "prospect_ai_assist_review_20260315_packets" / "manifest.json").read_text(encoding="utf-8")
            )
    ...
    def test_dump_deterministic_packet_slicing_with_caps(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self._write_cache_rows(
                ...
                [
                    ...
                    },
                    {
                        ...
                        "state": "TX",
                        "source": "aiha_consultants_listing:12",
                    }
                ],
            )
            self._write_cache_rows(
            ...
            self.assertIn("Atlas Safety", first_seed_one)
            self.assertIn("Bravo Safety", second_seed_one)
            self.assertIn("Charlie Safety Compliance Group", second_seed_one)

    def test_dump_state_lic_fit_scoring_prioritizes_consultant_like_rows_under_caps(self):
        with tempfile.TemporaryDirectory() as d:
            tmp = Path(d)
            data_dir = tmp / "runtime"
            output_dir = tmp / "prospect_ai_assist"
                ...
                [
                    {
                        ...
                        "website": "https://alpha.example.com",
                        "state": "TX",
                        "source": "aiha_consultants_listing:31",
                    },
                    {
                        "firm": "Bravo Safety",
                        "website": "https://bravo.example.com",
                        "state": "TX",
                        "source": "aiha_consultants_listing:32",
                    },
                ],
            )
            self._write_cache_rows(
            ...
            self.assertIn("AI_ASSIST_DUMP_OBSERVED_STATE_LIC_FIT_MISMATCH=1", text)
            self.assertIn("AI_ASSIST_DUMP_SOURCE_STATE_LIC_CANDIDATES=2", text)
    ...
    def test_import_persists_seed_id_provenance_when_seed_index_is_available(self):
        with tempfile.TemporaryDirectory() as d:
            tmp = Path(d)
            data_dir = tmp / "runtime"
            audit_dir = data_dir / "audits" / "prospect_ai_assist" / "prospect_ai_assist_review_20260315_packets"
            audit_dir.mkdir(parents=True, exist_ok=True)
            (audit_dir / "seed_index.json").write_text(
                json.dumps(
                    {
                        "seeds": {
                            "seed_abc123": {
                                "seed_source_token": "STATE_LIC",
                                "seed_source": "STATE_LIC",
                                "seed_source_url": "https://license.example.com/AC-100",
                                "source_record_id": "tdlr:AC-100",
                                "license_number": "AC-100",
                                "state_lic_license_class_norm": "electrical contractor",
                                "state_lic_hard_negative_class": "",
                                "state_lic_positive_families": ["safety", "credentials"],
                                "state_lic_negative_families": [],
            ...
            input_path = tmp / "prospect_ai_assist_review_20260315_packet_001_reviewed.csv"
            with open(input_path, "w", newline="", encoding="utf-8") as handle:
                writer = csv.DictWriter(handle, fieldnames=list(import_tool.REQUIRED_COLUMNS) + ["seed_id"])
                writer.writeheader()
                writer.writerow(
                    {
                        "state": "TX",
                        "decision": "accept",
                        "firm": "Safety Evidence Group",
                        "website": "https://safetyevidence.example.com",
                        "contact_name": "Taylor Safe",
                        "title": "Owner",
                        "email": "taylor@safetyevidence.example.com",
                        "source_urls": "https://safetyevidence.example.com/about",
                        "confidence": "93",
            ...
            env["DATA_DIR"] = str(data_dir)
            with mock.patch.dict(os.environ, env, clear=False), redirect_stdout(out):
                rc = import_tool.main(["--input", str(input_path), "--batch", "2026-03-15_AIASSIST_P001"])
            ...
            try:
                row = conn.execute(
                    f"""
                    SELECT seed_id, seed_source_token, source_record_id, license_number,
                           state_lic_license_class_norm, state_lic_positive_families_json
                    FROM {crm_store.AI_ASSIST_CANDIDATE_TABLE}
                    WHERE batch_id = '2026-03-15_AIASSIST_P001'
                    """
                ).fetchone()
            finally:
                conn.close()
            ...
            self.assertEqual(str(row["seed_id"] or ""), "seed_abc123")
            self.assertEqual(str(row["seed_source_token"] or ""), "STATE_LIC")
            self.assertEqual(str(row["source_record_id"] or ""), "tdlr:AC-100")
            self.assertEqual(str(row["license_number"] or ""), "AC-100")
            self.assertEqual(str(row["state_lic_license_class_norm"] or ""), "electrical contractor")
            self.assertEqual(json.loads(str(row["state_lic_positive_families_json"] or "[]")), ["safety", "credentials"])
